...
#include <math.h>
List<octree> collut::objects;
List<meshBuffer> collut::mesh;
List<SceneElement> collut::SEObjects;

void collut::registerObjectForCollision( SceneElement *e )
{
    meshBuffer *mb = e->getMesh();
    octree *oct = new octree( mb );
    objects.add(oct, e->getID() );
    mesh.add(mb,e->getID() );
    SEObjects.add(e, e->getID() );
