...
std::unique_ptr<float *> &&meshBuffer::GetVertexDataArray() const {
    auto *vertexData = new float[nvertices * 3 + ntexcoords * 2 + nnormals * 4];
    for (int i = 0; i < nfaces; i++) {
        memcpy(vertexData, vertices[faces[i].vertex[0]].coord, 3 * sizeof(float));
        memcpy(vertexData, texcoords[faces[i].texUV[0]].coord, 2 * sizeof(float));
        memcpy(vertexData, normals[faces[i].vnormal[0]].coord, 3 * sizeof(float));

        memcpy(vertexData, vertices[faces[i].vertex[1]].coord, 3 * sizeof(float));
        memcpy(vertexData, texcoords[faces[i].texUV[1]].coord, 2 * sizeof(float));
        memcpy(vertexData, normals[faces[i].vnormal[2]].coord, 3 * sizeof(float));

        memcpy(vertexData, vertices[faces[i].vertex[2]].coord, 3 * sizeof(float));
        memcpy(vertexData, texcoords[faces[i].texUV[2]].coord, 2 * sizeof(float));
        memcpy(vertexData, normals[faces[i].vnormal[3]].coord, 3 * sizeof(float));
    }
