...
SDL_Surface *GLAux::screen;
SDL_Event GLAux::event;
Uint8 *GLAux::Keystate;
...
void GLAux::draw2DObject( plane2D area, Texture *texture, float cor[4], float rect[4] )
{
    ...
    glTexCoord2f(rect[0], rect[1]); glVertex3f( -area.w/2, -area.h/2, 0);	// Bottom Left
    glTexCoord2f(rect[2], rect[1]); glVertex3f(  area.w/2, -area.h/2, 0);	// Bottom Right
    glTexCoord2f(rect[2], rect[3]); glVertex3f(  area.w/2,  area.h/2, 0);	// Top Right
    glTexCoord2f(rect[0], rect[3]); glVertex3f( -area.w/2,  area.h/2, 0);	// Top Left
    glEnd();
    glLoadIdentity();
...
void GLAux::drawSphere( vector3D position, double radius )
{
...
void GLAux::rayFromScreenCoord(int scr_x, int scr_y, vector3D &point, vector3D &dir)
{
    ...
    double zNear  = 1.0;
    double zFar   = 1.0;
    double aspect = (GLfloat)WIDTH/(GLfloat)HEIGHT;
    ...
    while ( SDL_PollEvent(&event) ) {
        switch (event.type) {
            ...
            case SDL_MOUSEBUTTONDOWN:
                ...
                if(GLAux::event.button.button==SDL_BUTTON_WHEELDOWN)
                    mouse_wheel = -20;
                if(GLAux::event.button.button==SDL_BUTTON_WHEELUP)
                    mouse_wheel = 20;
