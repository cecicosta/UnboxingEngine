...
void Labirinto::ronald_lab()
{
    ...
    int escolha[4];
    int cont=0;
    //Cria��o da pilhas usadas para guardar os caminhos percorridos pelo labirinto.
    List<Matriz::node*> *caminho_1 = new List<Matriz::node*>();
    List<Matriz::node*> *caminho_2 = new List<Matriz::node*>();

    Matriz::node *aux[6]; //Vetor de n�s auxiliares.
    ...
    //La�o que define o percurso feito desde a entrada e desde a saida atravez do labirinto.
    while(aux[3]->cima->valor!=j&&aux[3]->cima->valor!=j&&aux[3]->cima->valor!=j&&aux[3]->cima->valor!=j)
    {

        //La�o para subir a pilha caso o caminho esteja trancado.
        do
        {

        aux[3]->valor=floor;

        //Condi��es de escolha da proxima celula a ser percorrida.
    ...
    //Percorre toda a matriz abrindo a celulas trancadas aplicando o percurso em cada uma at� que todo o labirinto esteja conectado.
    floor=4;
