...
glmFindMaterial(GLMmodel* model, char* name)
{
    ...
    /* XXX doing a linear search on a char* key'd list is pretty lame,
...
float *scaleVertex( float vertex[3], float scale[3], float novo[] )
{
    novo[0] = vertex[0]*scale[0];
    novo[1] = vertex[1]*scale[1];
    novo[2] = vertex[2]*scale[2];
    return novo;
}
 ...
 */
GLvoid
glmDraw(GLMmodel* model, GLuint mode, float scale[3])
{
    static GLuint i;
    ...
    while (group) {
        ...
        for (i = 0; i < group->numtriangles; i++) {
            ...
            float vertex[3];
            scaleVertex(&model->vertices[3 * triangle->vindices[0]], scale, vertex);
            glVertex3fv(vertex);
            ...
            scaleVertex(&model->vertices[3 * triangle->vindices[1]], scale, vertex);
            glVertex3fv(vertex);
            ...
            scaleVertex(&model->vertices[3 * triangle->vindices[2]], scale, vertex);
            glVertex3fv(vertex);
