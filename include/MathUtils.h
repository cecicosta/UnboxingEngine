...
public:
    ...
    template<typename T>
    static bool GaussJordan(const std::vector<T> &scalar, const std::vector<T> &constants, std::vector<T> &solution) {
...
private:
    ...
    template<typename T>
    static bool pivoting_row(std::vector<T> &m, std::map<int, int> &pivot_map, int pivoting_row) {
    ...
    template<typename T>
    static std::vector<T> create_identity_matrix(std::uint32_t n) {
