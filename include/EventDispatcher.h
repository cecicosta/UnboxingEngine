...
class to_specialized_vector {
    ...
    public:
        To& operator*() {return *dynamic_cast<To*>(std::vector<From>::iterator::operator*());}
    };
public:
...
protected:
    ...
    void RegisterListener(IListener<> &listener) {
        for (auto &&hash: listener.hash_handles) {
            if (m_Listeners.find(hash) == m_Listeners.end()) {
                to_specialized_vector<IListener<> *> first;
    ...
    template<typename T>
    to_specialized_vector<CListener*> GetListeners() {
...
private:
    std::unordered_map<std::size_t, to_specialized_vector<IListener<> *>> m_Listeners;
};
