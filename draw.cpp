...
//glEnable(GL_TEXTURE_2D);
    ...
    void draw::casa(float scale[]){
        ...
        //pontos telhado
        float t1[3] = {1.2*scale[0], 1*scale[1], 0};
        float t2[3] = {-0.2*scale[0], 1*scale[1], 0};
        float t3[3] = {-0.2*scale[0], 1*scale[1], -2*scale[2]};
        float t4[3] = {1.2*scale[0], 1*scale[1], -2*scale[2]};
        float t5[3] = {0.5*scale[0], 1.6*scale[1], 0};
        float t6[3] = {0.5*scale[0], 1.6*scale[1], -2*scale[2]};
    ...
    void draw::carroca(float scale[]){
        ...
        //pontos alsas
        float a1[3] = {0, 1*scale[1], -3.5*scale[2]};
        float a2[3] = {0, 0.9*scale[1], -3.5*scale[2]};
        float a3[3] = {0, 0.9*scale[1], -2*scale[2]};
        float a4[3] = {0, 1*scale[1], -2*scale[2]};
        float a5[3] = {1*scale[0], 1*scale[1], -3.5*scale[2]};
        float a6[3] = {1*scale[0], 0.9*scale[1], -3.5*scale[2]};
        float a7[3] = {1*scale[0], 0.9*scale[1], -2*scale[2]};
        float a8[3] = {1*scale[0], 1*scale[1], -2*scale[2]};

        float r1[3] = {-0.05*scale[0], 0, -1*scale[2]};
        float r2[3] = {1.05*scale[0], 0, -1*scale[2]};
        float rr[3] = {0,0.8*scale[1],0.8*scale[2]};
        glPushMatrix();
        //glEnable(GL_CULL_FACE);
        glBegin(GL_QUADS);
            //frente
...
*/
        ...
        for(int j=0; j<24; j++)
        {
            ...
            for(int i=0; i<=12; i++)
            {
                ...
                glTexCoord2f(cos(RAD_90 - RAD_3_75*(j)), 0.5 - 0.5*sin(RAD_90 - RAD_7_5*i));
                ...
                glTexCoord2f(cos(RAD_90 - RAD_3_75*(j+1)), 0.5 - 0.5*sin(RAD_90 - RAD_7_5*i));
        ...
        for(int j=0; j<24; j++)
        {
            ...
            for(int i=0; i<=12; i++)
            {
                ...
                glNormal3f( sin(RAD_15*(j-1))*cos(RAD_90 + RAD_15*i), sin(RAD_90 + RAD_15*i), cos(RAD_90 + RAD_15*i)*cos(RAD_15*(j-1)));
                glVertex3f( radius*sin(RAD_15*(j-1))*cos(RAD_90 + RAD_15*i)*scale[0], (670.0f + radius*sin(RAD_90 + RAD_15*i))*scale[1]*0.3, radius*cos(RAD_90 + RAD_15*i)*cos(RAD_15*(j-1))*scale[2] );


                glTexCoord2f(cos(RAD_90 - RAD_3_75*(j+1)), 0.5 - 0.5*sin(RAD_90 - RAD_7_5*i));

                glNormal3f( sin(RAD_15*j)*cos(RAD_90 + RAD_15*i), sin(RAD_90 + RAD_15*i), cos(RAD_90 + RAD_15*i)*cos(RAD_15*j));
                glVertex3f( radius*sin(RAD_15*j)*cos(RAD_90 + RAD_15*i)*scale[0], (670.0f + radius*sin(RAD_90 + RAD_15*i))*scale[1]*0.3, radius*cos(RAD_90 + RAD_15*i)*cos(RAD_15*j)*scale[2] );
            }
            glEnd();
        }
        ...
        for(float i=-1.0f; i<1.0f; i = i + inc)
        {
            for(int j=0; j<24; j++)
            {
                float p1[] = { (pow(i+0,2)*sin(RAD_15*(j+0)) + r*sin(RAD_15*(j+0)))*scale[0], (i+0)*scale[1] ,  (pow(i+0,2)*cos(RAD_15*(j+0)) + r*cos(RAD_15*(j+0)))*scale[2]};
                float p2[] = { (pow(i+0,2)*sin(RAD_15*(j+1)) + r*sin(RAD_15*(j+1)))*scale[0], (i+0)*scale[1] ,  (pow(i+0,2)*cos(RAD_15*(j+1)) + r*cos(RAD_15*(j+1)))*scale[2]};
                float p3[] = { (pow(i+inc,2)*sin(RAD_15*(j+0)) + r*sin(RAD_15*(j+0)))*scale[0], (i+inc)*scale[1] ,  (pow(i+inc,2)*cos(RAD_15*(j+0)) + r*cos(RAD_15*(j+0)))*scale[2]};
                float p4[] = { (pow(i+inc,2)*sin(RAD_15*(j+1)) + r*sin(RAD_15*(j+1)))*scale[0], (i+inc)*scale[1] ,  (pow(i+inc,2)*cos(RAD_15*(j+1)) + r*cos(RAD_15*(j+1)))*scale[2]};

                float n1[] = { pow(i+0,2)*sin(RAD_15*(j+0)) + r*sin(RAD_15*(j+0)), 0 ,  pow(i+0,2)*cos(RAD_15*(j+0)) + r*cos(RAD_15*(j+0))};
                float n2[] = { pow(i+0,2)*sin(RAD_15*(j+1)) + r*sin(RAD_15*(j+1)), 0 ,  pow(i+0,2)*cos(RAD_15*(j+1)) + r*cos(RAD_15*(j+1))};
                float n3[] = { pow(i+inc,2)*sin(RAD_15*(j+0)) + r*sin(RAD_15*(j+0)), inc ,  pow(i+inc,2)*cos(RAD_15*(j+0)) + r*cos(RAD_15*(j+0))};
                float n4[] = { pow(i+inc,2)*sin(RAD_15*(j+1)) + r*sin(RAD_15*(j+1)), inc ,  pow(i+inc,2)*cos(RAD_15*(j+1)) + r*cos(RAD_15*(j+1))};

                float t1[] = { 0.0208333*j, 0.5 + 0.25*(1+i)};
                float t2[] = { 0.0208333*(j+1), 0.5 + 0.25*(1+i)};
                float t3[] = { 0.0208333*j, 0.5 + 0.25*(1+i+inc)};
                float t4[] = { 0.0208333*(j+1), 0.5 + 0.25*(1+i+inc)};
    ...
    void draw::torre(float scale[]){
                ...
                float t3[3] = {-0.2*scale[0], 1*scale[1], -2*scale[2]};
                float t4[3] = {1.2*scale[0], 1*scale[1], -2*scale[2]};
                float t5[3] = {0.5*scale[0], 1.6*scale[1], -1*scale[2]};
                float t6[3] = {0.5*scale[0], 1.6*scale[1], -1*scale[2]};
    ...
    void draw::arvore(float scale[])
    {
        ...
        {
            float p1[3] = {R*cos((alfa+180)*M_PI/180)*scale[0], 0, R*sin((alfa+180)*M_PI/180)*scale[2]};
            float p2[3] = {R*cos((alfa)*M_PI/180)*scale[0], 0, R*sin((alfa)*M_PI/180)*scale[2]};
            float p3[3] = {R*cos((alfa)*M_PI/180)*scale[0], R*scale[1], R*sin((alfa)*M_PI/180)*scale[2]};
            float p4[3] = {R*cos((alfa+180)*M_PI/180)*scale[0], R*scale[1], R*sin((alfa+180)*M_PI/180)*scale[2]};
