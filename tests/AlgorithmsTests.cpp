...
template <typename T, int dimension>
SCollisionResult<T, dimension> findPathIntersectionWithSegment(const std::vector<Vector<T, dimension>> &vertices, const Vector<T, dimension> &start, const Vector<T, dimension> &end, bool connectLastVertex = true) {
...
bool CheckBoxToBoxCollision(std::vector<Vector<T, dimension>> box1, std::vector<Vector<T, dimension>> box2, const Vector<T, dimension> &direction, SCollisionResult<T, dimension> &result) {
    ...
    for (int v = 1; v < 4; v++) {
        ...
        auto escape;
        auto normal;
        result = findPathIntersectionWithSegment(box1, c, d);
...
TEST(GeneralCollisionHelpers, TraceTrajectoryIntoBox_ValidateIntersectionResults) {
    ...
    Vector2f start(-11, 6);
    Vector2f end(-6, 8);
    auto result = MyCollision::findPathIntersectionWithSegment<float, 2>(box, start, end);
